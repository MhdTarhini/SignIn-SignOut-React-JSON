...
function SignUp() {
    ...
    const [formValues, setFormValues] = useState({
      ...
      Password: "",
      category: [],
    }); // Initial value of information form of unique user
    const [formErrors, setFormErrors] = useState({}); // Errors of form fill
    const [submit, setSubmit] = useState(false); // form submit
    const { Category } = UseFetchCategory(); //Fetch category from json-file
    const { User } = UseFetchUser(); // fetch User (POST , PUT) from json-file
    const navigate = useNavigate(); // from REct-Router-Dom
